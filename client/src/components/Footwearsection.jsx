...
const Footwearsection = React.memo(({filterData, search}) => {
  ...
  useEffect(()=>{
    if(token){
      dispatch(getproductsbycategory({token, category:'footwear'}))
    }
  },[token, dispatch]);
...
<div className="col" key={index}>
          <Link className="card h-100" to={`/description/${product?._id}`}>
            <img
src={loading ? imageloader : `http://localhost:3000/${product?.images[0]}`}
...
Footwearsection.displayName = 'Footwearsection'
Footwearsection.propTypes={
